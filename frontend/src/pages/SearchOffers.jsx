...
export default function SearchOffers() {
  ...
  useEffect(() => {
    ...
    .then(data => {
      if (Array.isArray(data)) {
        ...
        setOthers(oth);
        setFilteredRec(rec);
        setFilteredOthers(oth);
        setOffers([...rec, ...oth]);
        setFiltered([...rec, ...oth]);
        setHasRecommendations(rec.length > 0);
      }
      setLoading(false);
    })
  ...
  useEffect(() => {
    ...
    if (hasRecommendations) {
      const fr = applyFilters(recommended);
      const fo = applyFilters(others);
      setFilteredRec(fr);
      setFilteredOthers(fo);
      setFiltered([...fr, ...fo]);
    } else {
      setFiltered(applyFilters(offers));
    }
  }, [search, wilaya, skill, type, offers, recommended, others, hasRecommendations]);

  const handleApply = (offerId) => { setCoverModal(offerId); setCoverLetter(""); };
  ...
  const handleSubmitApplication = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setCoverModal(null); setCoverLetter("");
      } else {
  ...
  function OfferCard({ offer, i, isRecommended }) {
      ...
      <PageShell title="Search Offers" subtitle="✦ FIND YOUR INTERNSHIP" onMenuClick={() => setOpen(true)}>
        ...
        <div style={{ background: colors.white, borderRadius: "16px", padding: "24px 28px", marginBottom: "28px", boxShadow: "0 4px 20px rgba(17,34,80,0.07)", position: "relative", overflow: "hidden", animation: "fadeUp .4s ease both" }}>
          ...
          <div style={{ fontSize: "12px", color: "#bbb", marginTop: "12px", letterSpacing: ".3px" }}>
            ...
            {hasRecommendations && filteredRec.length > 0 && (
              <span style={{ color: colors.gold, marginLeft: "8px" }}>⭐ {filteredRec.length} match your skills</span>
            )}
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div>
            {filteredRec.length > 0 && (
              <div style={{ marginBottom: "36px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(320px,1fr))", gap: "20px" }}>
                  {filteredRec.map((offer, i) => <OfferCard key={offer.id} offer={offer} i={i} isRecommended={true} />)}
                </div>
              </div>
            )}
            {filteredOthers.length > 0 && (
              <div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(320px,1fr))", gap: "20px" }}>
                  {filteredOthers.map((offer, i) => <OfferCard key={offer.id} offer={offer} i={i} isRecommended={false} />)}
                </div>
              </div>
            )}
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(320px,1fr))", gap: "20px" }}>
            {filtered.map((offer, i) => <OfferCard key={offer.id} offer={offer} i={i} isRecommended={false} />)}
          </div>
        )}
      ...
      {coverModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(17,34,80,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, backdropFilter: "blur(4px)", fontFamily: "Georgia, serif" }}>
          <div style={{ background: "#fff", borderRadius: "20px", padding: "40px", width: "100%", maxWidth: "520px", boxShadow: "0 30px 80px rgba(17,34,80,0.25)", position: "relative", overflow: "hidden", animation: "fadeUp 0.3s ease both" }}>
            ...
            <button onClick={() => { setCoverModal(null); setCoverLetter(""); }} style={{ position: "absolute", top: "16px", right: "16px", background: "none", border: "none", cursor: "pointer", fontSize: "20px", color: "#aaa", lineHeight: 1 }}>✕</button>
            ...
            <div style={{ display: "flex", gap: "12px" }}>
              <button onClick={() => { setCoverModal(null); setCoverLetter(""); }}
