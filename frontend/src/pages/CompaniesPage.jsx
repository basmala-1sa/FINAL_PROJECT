...
function ReviewsModal({ company, onClose }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(17,34,80,0.5)", backdropFilter:"blur(6px)", zIndex:9999, display:"flex", alignItems:"center", justifyContent:"center", padding:"20px" }} onClick={onClose}>
      <div style={{ background:C.white, borderRadius:"24px", width:"100%", maxWidth:"600px", maxHeight:"80vh", overflow:"hidden", display:"flex", flexDirection:"column", boxShadow:"0 32px 80px rgba(17,34,80,0.25)", animation:"fadeUp .3s ease both" }} onClick={e=>e.stopPropagation()}>

        {/* Header */}
        <div style={{ padding:"28px 28px 20px", borderBottom:"1px solid rgba(17,34,80,0.07)", position:"relative" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            <div style={{ display:"flex", alignItems:"center", gap:"14px" }}>
              <div style={{ width:"48px", height:"48px", borderRadius:"12px", background:`linear-gradient(135deg,rgba(194,160,114,0.2),rgba(194,160,114,0.05))`, border:"1px solid rgba(194,160,114,0.3)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"20px", fontWeight:"700", color:C.gold, fontFamily:"'Cormorant Garamond',Georgia,serif" }}>
                {company.company_name[0].toUpperCase()}
              </div>
              <div>
                <h2 style={{ margin:0, fontSize:"18px", fontWeight:"700", color:C.navy }}>{company.company_name}</h2>
            ...
            <button onClick={onClose} style={{ background:"rgba(17,34,80,0.06)", border:"none", borderRadius:"50%", width:"36px", height:"36px", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", color:C.navy }}>
              <FiX size={16}/>
            </button>
          </div>
        </div>
        ...
        <div style={{ overflowY:"auto", padding:"20px 28px", flex:1 }}>
          {loading ? (
            ...
            <div key={i} style={{ background:C.offWhite, borderRadius:"14px", padding:"18px 20px", marginBottom:"12px", border:"1px solid rgba(17,34,80,0.06)" }}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"10px" }}>
                <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
                  <div style={{ width:"34px", height:"34px", borderRadius:"50%", background:`linear-gradient(135deg,${C.gold},${C.lightGold})`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:"13px", fontWeight:"700", color:C.navy, fontFamily:"'Cormorant Garamond',Georgia,serif" }}>
...
export default function CompaniesPage() {
  ...
  return (
    <div style={{ fontFamily:"Georgia,serif", background:C.cream, minHeight:"100vh", color:C.navy }}>
        ...
        <div ref={heroRef} style={{ maxWidth:"700px", margin:"0 auto", textAlign:"center", position:"relative", zIndex:1 }}>
          ...
          <div style={{ position:"relative", maxWidth:"440px", margin:"0 auto", animation:heroVisible?"fadeUp .6s ease .3s both":"none" }}>
            ...
            <input
              ...
              style={{ width:"100%", padding:"14px 16px 14px 44px", borderRadius:"10px", border:"1.5px solid rgba(194,160,114,0.3)", background:"rgba(255,255,255,0.08)", color:C.white, fontSize:"14px", fontFamily:"Georgia,serif", outline:"none", backdropFilter:"blur(8px)", transition:"border-color .2s" }}
      ...
      {/* Stats bar */}
      <div style={{ background:C.white, padding:"20px 5%", borderBottom:"1px solid rgba(17,34,80,0.06)", display:"flex", alignItems:"center", justifyContent:"center", gap:"40px" }}>
